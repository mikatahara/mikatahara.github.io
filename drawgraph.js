...
function triangle(ctx, p, hight) {
	...
	ctx.beginPath();
	ctx.moveTo(dx, dy);
	ctx.lineTo(dx + hight / 2, dy + hight);
	ctx.lineTo(dx - hight / 2, dy + hight);
	ctx.lineTo(dx, dy);
	ctx.closePath();
	ctx.stroke();
...
DrawGraph.prototype = {
	...
	fConvPos: function (x, y) {
		...
		this.iPx = Math.min(this.iWxe,Math.max(this.iWxs,this.iPx));
		this.iPy = Math.min(this.iWye,Math.max(this.iWys,this.iPy));
	},
	...
	/* *******************************************	*/
	/*				文字のフォント設定				*/
	/*	fSetFont("28px 'ＭＳ ゴシック'")			*/
	/*	"28px 'ＭＳ ゴシック'"						*/
	/*	"bold 14px 'ＭＳ 明朝'"						*/
	/*	"italic bold 20px sans-serif"				*/
	/* *******************************************	*/
	...
	fDrawArcXYnofill: function (x, y, radix, sp = 0, ep = Math.PI * 2) {
		this.ctx.beginPath();
		this.fConvPos(x, y);
		this.ctx.arc(this.iPx, this.iPy, radix, sp, ep, false);
		this.ctx.stroke();
	},
